...
class RP():
    ...
    def rp(self, x):
        ...
        x = kornia.geometry.resize(x, size=(rnd, rnd))
...
class AverageBlur():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.filters.box_blur(x,  (self.ksize, self.ksize))
        return x
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            if np.random.rand() < 0.5:
                angle = -angle
            x = kornia.geometry.rotate(x, angle=angle.to(x.device, dtype=torch.float32))
        return x
...
class RandShear():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            x = kornia.geometry.shear(x, shear.to(device=x.device, dtype=x.dtype))
        return x
...
class RandTranslate():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            x = kornia.geometry.translate(x, translate)
        return x
...
class NonLocalMean():
    ...
    def __call__(self, x):
        ...
        if np.random.rand() < self.p:
            ...
            datas = kornia.color.bgr_to_rgb(datas)
...
#             self.aug = iaa.Affine(scale={"x": (lr=0.5, 1.5), "y": (lr=0.5, 1.5)})
#
#             datas = kornia.tensor_to_image(x)
#             datas = np.round(datas * 255)
#             datas = datas.astype(np.uint8)
